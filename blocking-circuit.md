...
また、
$$
v_{L1}(t)=V_{CC}-V_{CE(SAT)}\tag{vl1thalf}
$$
であり、時間によらず一定値であるので、
$$
\begin{cases}
...
&=&\frac{V_{CC}-V_{CE(SAT)}}{L}\cdot T_1 \tag{iccoil}
\end{cases}
$$
となる。
...
(2),vl1thalf式より、
$$
v_{L2}(t)=-(V_{CC}-V_{CE(SAT)}) \tag{vl2t}
$$
...
だから、{vl2t}式を代入して、
$$
I_b=\frac{2V_{CC}-V_{CE(SAT)}-V_{be}}{R_b} \tag{Ib}
$$
$T_r$が${\small \textrm{ON}}$時であり、
$V_{CE(SAT)}\ll \textrm{LED} $の順方向電圧なので、$R_c$に流れる電流は無視できるため、$T_r$の特性より、
$$
I_c=h_{FE}\cdot I_b \tag{Ic}
$$


{Ib},{Ic}式より
$$
I_c=h_{FE}\cdot \frac{2V_{CC}-V_{CE(SAT)}-V_{be}}{R_b} \tag{ictr}
$$

よって {iccoil}, {ictr}式から、$T_1$は、
$$
...
$$
\begin{cases}
V_{CE(SAT)} \simeq 0 \tag{approximation} \\
V_{BE} \simeq 0
\end{cases}
$$
と近似した場合、
...
{t1}式から、
$$
T_1 \simeq h_{FE}\cdot \frac{2L}{R_b} \tag{t1}
$$
となる。
...
&=& I_c \cdot R_c \cdot \frac{1}{s + \frac{R_c}{L_1}} \tag{vl1s}
\end{cases}
$$
ここで、
$$
...
$$
だから、
{vl1s}式を逆ラプラス変換して時間領域に戻すと
$$ v_{L1}(t)=I_c \cdot R_c \cdot e^{-\frac{R_c}{L_1}t} \tag{vl1t} $$
となる。
$$
\begin{cases}
