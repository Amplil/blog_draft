# ブロッキング回路の原理
## モデルの定義
下図のように各点での電圧値および電流値を考える（ベクトルの向きはすべて上向きにしました）。

<img src="https://electronic-handicraft.work/wordpress/wp-content/uploads/2021/07/ブロッキング回路図1-1.png" width=60%>

コイル\\(L\_1\\)と\\(L\_2\\)は巻数比１、結合係数１の理想トランスとする。すなわち、ここでのトランスの結合条件は
\begin{cases}
L\_1=L\_2=L\\\\
n\_1:n\_2=1:1\\\\
(結合係数) \\ K=1
...
である。また、トランジスタ\\(T\_r\\)は下の3つの独立な定数のみで定義する。
\begin{cases}
h\_{FE} &: 直流電流増幅率\\\\
